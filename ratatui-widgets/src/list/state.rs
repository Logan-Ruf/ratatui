...
impl ListState {
    ...
    ///
    /// Until the list is rendered, the number of items is not known. In this case,
    ...
    /// Selects the first item or `None`.
    ///
    /// Sets the `selected` index to `Some(0)`, unless the `item_count` is zero, then `selected` is set
    /// to `None`.
    ///
    /// # Examples
    ///
    ...
    /// Scrolls up by increasing `selected` index by the specified `amount`.
    ///
    ...
    /// Scrolls up by decreasing `selected` by the specified `amount`.
    ///
    /// If the `amount` causes `selected` to go below zero, it will clamp.
    ///
    /// Returns `true` if the `selected` index was successfully moved, `false` if it was clamped.
    ///
    /// # Examples
